...
public class CatChunkGenerator implements IChunkGenerator {

    private final World worldObj;
    private Random random;
    private Biome[] biomesForGeneration = { ModBiomes.biomecat };

    // private List<Biome.SpawnListEntry> mobs = Lists.newArrayList(new
    // Biome.SpawnListEntry(EntityWeirdZombie.class, 100, 2, 2));

    private MapGenBase caveGenerator = new MapGenCaves();
    private NormalTerrainGenerator terraingen = new NormalTerrainGenerator();

    public CatChunkGenerator(World worldObj) {
    ...
    public Chunk generateChunk(int x, int z) {
        ...
        // this.biomesForGeneration =
        // this.worldObj.getBiomeProvider().getBiomes(this.biomesForGeneration, x * 16,
        // z * 16, 16, 16);
